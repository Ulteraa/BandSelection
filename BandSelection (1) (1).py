...
def Test_set (Batch_Size):
        ...
        filenames, labels = Load_Dataset_test()
        Class_number = 331
...
def DataSet(Batch_Size):
 filenames, labels=  Load_Dataset_train()
 Class_number=331
 label=one_hot_label(labels,Class_number)
 A=tf.constant(filenames)
 B=tf.constant(label,tf.float32)
...
train_x = tf.placeholder(tf.float32, [None,224,224,9])#this part should be modified when you wanna evaluate your algorithm band by band
